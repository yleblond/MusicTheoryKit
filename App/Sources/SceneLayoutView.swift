...
private struct SceneRoleRow: View {
    ...
    /// can simply disable itself instead of surfacing an error for an expected state. Reads
    /// the attached TRACK's own live `soundEnabled`, not `role.soundEnabled` — see
    /// `testSceneRoleSound`'s doc comment for why those two can drift.
    private var canTestSound: Bool {
        guard let trackID = role.attachedTrackID else { return false }
        return session.tracks.first { $0.id == trackID }?.soundEnabled ?? false
    }
